...
//	lay tong so thanh vien
	...
	public function getall_m_wallet() {
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_m_wallet as A INNER JOIN " . DB_PREFIX ."customer  as B on A.customer_id=B.customer_id WHERE A.amount > 0");
			return $query->rows;
	}
	public function update_wallet_m_0($customer_id){
		$query = $this -> db -> query("
	...
	public function getgoidautu($customer_id){
		$query = $this -> db -> query("
			SELECT MAX(filled) as filled  FROM ". DB_PREFIX . "customer_provide_donation
